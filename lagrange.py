...
class Lagrangiano(Scene):
    def construct(self):
        ...
        lag_text = TexMobject("\\text{Lagrangiano}", color=RED).next_to(lag, UP*2).scale(2)
        
        
        dummyLHS = TexMobject("\\mathcal{L}", color=YELLOW).shift(UP*.2).scale(1.5)
        ...
        el_text= TexMobject("\\text{Ecuación de Euler-Lagrange}", color=RED).scale(2).next_to(el_eqns,2*UP)
        ...
        explain = TextMobject(" Para cada componente vectorial $i$", color=YELLOW)
        ...
        equality_text = TextMobject("Como una ecuación vectorial", color=YELLOW)
        ...
        n2 = TextMobject(" $2^{da}$ ley de Newton", color=RED).next_to(g, UP).shift(DOWN*.5).scale(1.5)
